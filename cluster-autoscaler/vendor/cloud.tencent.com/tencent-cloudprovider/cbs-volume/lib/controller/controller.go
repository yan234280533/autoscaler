...
import (
	...
	"cloud.tencent.com/tencent-cloudprovider/cbs-volume/lib/leaderelection"
	rl "cloud.tencent.com/tencent-cloudprovider/cbs-volume/lib/leaderelection/resourcelock"
	"k8s.io/api/core/v1"
...
func NewEventRecorder(client kubernetes.Interface, provisionerName string, identity string) (record.EventRecorder) {
	...
	if err != nil {
		eventRecorder = broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: fmt.Sprintf("%s %s", provisionerName, identity)})
	} else {
		eventRecorder = broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: fmt.Sprintf("%s %s %s", provisionerName, strings.TrimSpace(string(out)), identity)})
	}
...
func (ctrl *ProvisionController) Run(stopCh <-chan struct{}) {
	glog.Infof("Starting provisioner controller %s!", ctrl.identity)
